...
class SoundGenerator:
    ...
    def combineSounds(self, soundObj, operator="+"):
        ...
        newFrequency = (
            int(self.getFrequency())
            * int(soundObj.getFrequency())
    ...
    def __pow__(self, soundObj):
        ...
        minSoundObj = SoundGenerator(
            waveType="Temp",
            frequency=100,
            amplitude=np.max(minSound),
            duration=newDuration,
        )
        minSoundObj.setSound(minSound)
        maxSoundObj = SoundGenerator(
            waveType="Temp",
            frequency=100,
            amplitude=np.max(maxSound),
            duration=newDuration,
        )
    ...
    def shiftBy(self, numberOfSamples):
        ...
        retSoundObj = SoundGenerator(
            self.waveType, self.frequency, self.amplitude, self.duration
        )
...
if __name__ == "__main__":
    ...
    sqr1 = SoundGenerator(
        waveType="Square", frequency=400.0, amplitude=0.2, duration=3.0
    )
    sqr2 = SoundGenerator(
        waveType="Square", frequency=500.0, amplitude=0.2, duration=3.0
    )
    saw1 = SoundGenerator(waveType="Sawtooth", duration=3)

    note1 = SoundGenerator(
        waveType="Square", frequency=400.0, amplitude=0.2, duration=5.0
    )
    note2 = SoundGenerator(
        waveType="Square", frequency=450.0, amplitude=0.2, duration=5.0
    )
    note3 = SoundGenerator(
        waveType="Square", frequency=600.0, amplitude=0.2, duration=5.0
    )
    note4 = SoundGenerator(
        waveType="Square", frequency=700.0, amplitude=0.1, duration=5.0
    )
    note5 = SoundGenerator(
        waveType="Square", frequency=800.0, amplitude=0.1, duration=5.0
    )
